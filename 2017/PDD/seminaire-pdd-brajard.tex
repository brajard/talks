...
%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Examples of forecast}
Examples of TSM
\end{frame}
...
number of data
efficiency of new methods (cf bengio)
\end{frame}

%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Example of the multilayer perceptron}
reprende le cours sur le perceptron
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{Typical problems of machine learning methods...}
Reprendre les exemples de base de bengio
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{One result : prediction of TSM}
\end{frame}
...
the probability of each state possibly taken by $X$. 
...
\item In the continuous case: $f_{X,Y}=f_X(x)f_Y(y)$ and therefore $f_{Y|X=x}(y) = f_Y(y)$.
\end{itemize}
...
\item if $X$ is continuous: $\E[g(X)]=\int_x g(x)f_X(x)dx$
\end{itemize}
\end{block}
In particular:
\begin{itemize}
...
\text{exp}\left(-\frac{1}{2}(\bm{x}-\bm{\mu})^T\bm{\Sigma}^{-1}(\bm{x}-\bm{\mu})\right)
$$
...
$$
\end{alertblock}
There are equivalent formulation when it involves continuous random variables.
For example if $Y$ is continuous:
$$
...
\vspace{-0.8em}
\begin{enumerate}
\item<1-> Let's consider 2 random variables $X$ and $Y$ related to a randomly picked individual
...
\item<2-> We know the Probability mass function of $X$
\begin{tabular}{|c|c|c|}
\hline
$x$ & 0 & 1 \\
\hline
...
\hline
\end{tabular}
\item<3-> We know the conditional density probability functions $f_{Y|X=i}(y)$
