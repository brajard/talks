...
\title[data assimilation]{Data assimilation.}
...
\pause
\begin{itemize}
\item<3-> Error on the observations \tikz[na] \node [coordinate] (n1) {};
\end{itemize}
\begin{align}
\mathcal{J}(\bm{x_0}) & =  
...
\end{align}
\begin{itemize}
\item<4->  Error on the background \tikz[na] \node
  [coordinate] (n2) {};
\item<5-> Model is supposed to be "perfect" (no model error):$\bm{x_k} = \bm{M_k}\bm{x_{k-1}}$
\end{itemize}
...
\end{block}
\pause
\begin{alertblock}{Impossible (very difficult) to compute if:}
\begin{itemize}
...
\node (x0) {$\bm{x_0}$};
\node [box,minimum width=7em,minimum height=2em] (M) [right=of x0]{\alt<3>{\red{Direct model}}{$\bm{M_{k,0}}$}} edge [<-] (x0);
...
\path [->,draw] (Jx0) |- (adjy);
\node (dy) [left=of adjy] {$\Delta_y$} edge [<-] (adjy);
\node [box,minimum width=7em,minimum height=2em] (adj) [left=of dy]{\alt<3>{\red{Adjoint model}}{$\bm{M_{k,0}}^T$}} edge [<-] (dy) ; 
...
Estimation of the round-off error during the direct and adjoint
simulation: CADNA library
\end{alertblock}
\end{frame}
...
\column{.5\textwidth}
\begin{figure}
\includegraphics[width=.95\textwidth]{./fig/adj-ieee.png}
\caption{Without CADNA}
\end{figure}
\pause
\column{.5\textwidth}
\begin{figure}
\includegraphics[width=.95\textwidth]{./fig/adj-cadna.png}
\caption{With CADNA}
\end{figure}
\end{columns}
\end{frame}
...
\caption{Water height at t=3 month}
\end{figure}
\column{.5\textwidth}
\begin{itemize}
...
\item Control parameter : initial height $h_0$
\item Observations : 
\begin{itemize}
...
\framesubtitle{running time : 10hours on Cluster CICLAD ( 640 AMD core 2.4GHz) 24 nodes}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{./fig/xb_fin.jpg}\\
\caption{Background field at t=3 months}
\end{figure}

\column{.3\textwidth}
\begin{figure}
\centering
\includegraphics[width=\textwidth]{./fig/xt_fin.jpg}\\
\caption{True field at t=3 months}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{./fig/xa_fin.jpg}\\
\caption{Analysed height field at t=3 months}
