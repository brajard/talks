...
\item Dot product between two vectors : $\bm{x}^T\bm{y}$
...
$v$ is not unique and is defined to a multiplicative constante. Usually, the eigen vector is considered
such as $\|v\| =1$.
\vspace{1em}
...
Elements in $\bm{D}$ are called the \alert{singular values}
...
\item if $n>m$, $\bm{x}=\bm{A}^+\bm{b}$ is the solution of the system with minimal $L^2$ norm ($\|\bm{x}\|_2$ is minimal), $\bm{A}^+=\bm{A}^T(\bm{AA}^T)^{-1}$
\item if $n<m$,  $\bm{x}=\bm{A}^+\bm{b}$ is such as $\bm{A}\bm{x}$ is as close as possible to $b$, $\bm{A}^+=(\bm{A}^T\bm{A})^{-1}\bm{A}^T$
