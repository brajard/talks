...
sequentially each time an instruction is read by the \alert{interpreter}.
\end{frame}
...
\item A basic Python IDE (we propose to use \verb|spyder|)
\item A "fancy" Python environment (\verb|ipython notebook|)
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth]{fig/fig_console_python.png}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth]{fig/fig_script.png}
\end{figure}
\end{columns}
\end{frame}
...
\begin{block}{}
A notebook (run in a web navigator) contains "cells" to execute python codes but also text, mathematics,
plots, etc.
\end{block}
\begin{figure}
\includegraphics[width=0.9\textwidth]{fig/notebook.png}
\end{figure}
\end{frame}
...
\verb|%run| command
\item in iPython notebook clicking on play, with \texttt{Ctrl+Enter} or with \texttt{Shift+Enter}
...
\begin{itemize}
\item Very familiar (use of terminal, shell commands)
\item Basic knowledge (user but mainly with graphic interface)
\item Almost never used yet.
\end{itemize}
\item Programming languages:
\begin{itemize}
\item Compiled language (C, Fortran)
\item Interpreted language (Matlab, Octave, Scilab , Python)
\item Specialized/high level language (php, R, Excel)
\item Language oriented (java, C++)
\end{itemize}
\item knowledge of Python
\begin{itemize}
\item Advanced user (use of toolbox, plot, script, class definition)
\item Beginners (basics of the language)
\item Never used Python.
\end{itemize}
