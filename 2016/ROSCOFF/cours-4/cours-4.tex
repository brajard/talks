...
$$
J(\bm{\theta}) =\frac{1}{2} \sum (y_j - \bm{\theta}^T\bm{x_j})^2
$$
...
As for the univariate linear regression, find $\bm{\hat{\theta}} = (\theta_0,\ldots,\theta_p)^T$, such as:
$$
...
$$
where $\bm{x}_{i \cdot} = (x_{i1},\ldots,x_{in})^T$
\end{block}
...
\end{block}
\begin{itemize}
\item Note that the solution is the solution using the moore-penrose inverse of the system:
$$
\bm{X}^T \bm{\theta} = \bm{Y}
$$
...
Dont need the cost fucnction, only the gradient
The hypothesis model don't need to be linear (only differentiable)
Can be parelleized
\end{frame}
\end{document}