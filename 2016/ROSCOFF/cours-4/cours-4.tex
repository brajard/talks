...
\end{block}
\begin{itemize}[<+->]
\item Estimators are unbiased, $\E[\hat{\theta}_0] = \theta_0$ and $\E[\hat{\theta}_1] = \theta_1$
\item $\Var(\hat{\theta}_0)=\sigma^2\left( \frac{1}{n} + \frac{\overline{x}^2}{\sum(x_i - \overline{x})^2} \right)$
\item $\Var(\hat{\theta}_1)=\frac{\sigma^2}{\sum(x_i - \overline{x})^2)}$
...
$$
where $\bm{x_{\cdot j}} = (x_{0,j},\ldots,x_{p,j})^T$
\end{frame}
%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
